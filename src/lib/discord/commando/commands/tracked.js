...
exports.run = async (client, msg, [args]) => {
	...
	try {
		...
		monsters.forEach((monster) => {
			...
			const littleLeague = monster.little_league_ranking >= 4096 ? 'any' : `top${monster.little_league_ranking} (@${monster.little_league_ranking_min_cp}+)`
			message = message.concat(`\n**${monsterName}** form: ${formName} ${monster.distance ? `, distance: ${monster.distance}m` : ''} ${monster.timer ? `, Timer: ${monster.timer}min` : ''} iv: ${miniv}%-${monster.max_iv}% cp: ${monster.min_cp}-${monster.max_cp} level: ${monster.min_level}-${monster.max_level} stats: ${monster.atk}/${monster.def}/${monster.sta} - ${monster.max_atk}/${monster.max_def}/${monster.max_sta} greatPvp: ${greatLeague} ultraPvp: ${ultraLeague} littlePvp: ${littleLeague} gender:${client.utilData.genders[monster.gender].emoji}`)
		})
